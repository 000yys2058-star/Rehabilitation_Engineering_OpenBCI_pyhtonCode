...
**연결된 보드 정보 반환**: 게이트가 1대임을 보장하므로 그 보드가 곧 연결된 보드다.
연결 성공 시 `CONNECTED_BOARD = {'name','mac','rssi'}` 를 설정하고 화면에 표시한다.
- 1차시: CSV 파일명에 보드명, `board_name`/`board_mac` 열 기록
- 2차시: `.joblib` 메타데이터에 `board` 저장. 불러올 때 현재 보드와 다르면 경고
- 용어: `Ganglion-c57c` 는 BLE 의 Complete Local Name(AD type 0x09),
  통칭 **advertised name**. OpenBCI GUI 표시명과 동일.
